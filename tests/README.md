...
# Core tests (currently not operational)

Core tests take longer than any other Sumokoin tests, due to the high amount of computational work involved in validating core components.
...
To run only Sumokoin's core tests (after building):
...
To run only Sumokoin's crypto tests (after building):
...
# Functional tests (currently not operational)
...
```bash
sumokoind --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `sumokoin-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py

# Fuzz tests (currently not operational)
...
To run only Sumokoin's unit tests (after building):

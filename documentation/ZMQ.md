# The Current/Future Status of ZMQ in Sumokoin

## ZMQ Pub/Sub
Client `ZMQ_SUB` sockets must "subscribe" to topics before it receives any data.
This allows filtering on the server side, so network traffic is reduced. Sumokoin
...
matching supported by both Sumokoin and ZMQ. The `format`, `context` and `event`
...
The Sumokoin daemon will ensure that events prefixed by `chain` will be sent in
...
chain instead of the last block. The Sumokoin daemon also ensures that
...
The Sumokoin daemon will send a `txpool_add` pub exactly once for each
